...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Review sentiment based on Review_Score (positive if score > 4.0, negative otherwise)
    df['Review_Sentiment'] = np.where(df['Review_Score'] > 4.0, 'Positive', 'Negative')

    # Handle any potential missing values introduced during feature engineering
    df.fillna({'Price_Category': 'Unknown', 'Review_Sentiment': 'Unknown'}, inplace=True)
